...
> Venturalítica's quickstart is **fully local**. The example below uses
> a bundled toy policy; in production, edit your own
> `assessment_plan.oscal.yaml` (see the [Policy Authoring
> guide](/venturalitica-sdk/guides/policy-authoring/)).
...
with vl.monitor("manual_audit"):
    vl.enforce(
        ...
        policy="risks.oscal.yaml"  # Your OSCAL policy file
    )
```

:::note
The code above requires a `risks.oscal.yaml` file. Head to the [Full Lifecycle](/venturalitica-sdk/full-lifecycle/) guide (Step 2) for the complete copy-paste YAML, or jump straight to [Level 1: The Engineer](/venturalitica-sdk/academy/level1-policy/) to learn how to write one from scratch.
:::
