...
:::caution[Hosted SaaS forthcoming]
The hosted Venturalítica SaaS endpoint is not yet operational. The CLI
commands `vl pull`, `vl push`, and `vl login` require a SaaS endpoint
configured via `VENTURALITICA_SAAS_URL`. For v0.6.0 we recommend the
**local-mode workflow** (see [Quickstart](/venturalitica-sdk/quickstart/) and the
[Migrating guide](/venturalitica-sdk/migrating/)): the SDK runs fully offline
against a local `assessment_plan.oscal.yaml`. We will announce SaaS
availability in a future minor release.
:::
