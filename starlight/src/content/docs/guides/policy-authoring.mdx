...
Any metric registered in `METRIC_REGISTRY` can be used as a `metric_key`. See [Metrics Reference](/venturalitica-sdk/reference/metrics/) for the full list. Common ones:
...
The `input:dimension` prop tells the SDK which protected attribute to analyze. The value is an abstract name that gets resolved via [Column Binding](/venturalitica-sdk/guides/column-binding/):
...
See [Dashboard Guide](/venturalitica-sdk/guides/dashboard/) for details.
