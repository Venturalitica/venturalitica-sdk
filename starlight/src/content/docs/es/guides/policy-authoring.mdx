...
Cualquier métrica registrada en `METRIC_REGISTRY` puede usarse como `metric_key`. Consulta la [Referencia de Métricas](/reference/metrics/) para la lista completa. Las más comunes:
...
El prop `input:dimension` indica al SDK qué atributo protegido analizar. El valor es un nombre abstracto que se resuelve mediante [Vinculación de Columnas](/guides/column-binding/):
...
Consulta la [Guía del Dashboard](/guides/dashboard/) para más detalles.
