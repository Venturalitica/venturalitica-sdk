...
from venturalitica.integrations import auto_log, _log_mlflow, _log_wandb, _log_clearml, generate_report
...
def test_generate_report(sample_results):
    ...
    assert "# Governance Compliance Report" in report
...
def test_log_wandb_active(sample_results):
    with patch('wandb.run') as mock_run, \
         ...
         patch('wandb.log_artifact') as mock_log_art, \
        ...
        assert mock_run.summary["governance.C1"] == "PASS"
...
def test_log_clearml_active(sample_results):
    with patch('clearml.Task') as mock_task_cls:
        mock_results = [
            sample_results[0],
            ComplianceResult("F1", "Fail", "acc", 0.5, 0.4, "<", False, "low")
        ]
        mock_task = MagicMock()
