...
def calc_l_diversity(df: pd.DataFrame, **kwargs) -> float:
    ...
    missing_cols = [c for c in quasi_identifiers + [sensitive_attr] if c not in df.columns]
    if missing_cols:
...
def calc_data_minimization_score(df: pd.DataFrame, **kwargs) -> float:
    ...
    if not sensitive_cols:
        ...
        sensitive_cols = [c for c in df.columns 
                         if any(keyword in c.lower() 
                               for keyword in ['age', 'gender', 'race', 'ethnicity', 
                                             'health', 'medical', 'income', 'salary', 
                                             'phone', 'email', 'ssn', 'id'])]
