...
def calc_path_decomposition(
    ...
    **kwargs,
    ...
    # Validation
    missing_cols = [
        c for c in [protected_attr, outcome] + mediators if c not in df.columns
    ]
    if missing_cols:
    ...
    if len(groups) < 2:
        raise ValueError(
            f"Protected attribute must have at least 2 groups, found {len(groups)}"
        )
...
def calc_counterfactual_fairness(
    df: pd.DataFrame, protected_attr: str, outcome: str, **kwargs
) -> float:
    """
...
def calc_fairness_through_awareness(
    ...
    **kwargs,
    ...
    if relevant_features is None:
        relevant_features = [
            c for c in df.columns if c not in [protected_attr, outcome]
        ]
...
def calc_causal_fairness_diagnostic(
    ...
    **kwargs,
        ...
        for comparison, effect in diagnostic["path_decomposition"].items():
            if effect.direct_effect > 0.1:
                verdict_parts.append(
                    f"⚠️ High direct effect ({effect.direct_effect:.4f}) in {comparison}"
                )
            else:
    ...
    if (
        diagnostic["counterfactual_fairness"]
        and diagnostic["counterfactual_fairness"] > 0.15
    ):
        verdict_parts.append(
            f"⚠️ High counterfactual disparity ({diagnostic['counterfactual_fairness']:.4f})"
        )
    ...
    if isinstance(fta, dict) and "information_leakage_score" in fta:
        if fta["information_leakage_score"] > 0.3:
            verdict_parts.append(
                f"⚠️ Information leakage detected ({fta['information_leakage_score']:.4f})"
            )
