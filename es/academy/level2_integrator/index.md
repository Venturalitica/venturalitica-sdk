...
**Prerrequisito**: [Nivel 1 (El Ingeniero)](https://venturalitica.github.io/venturalitica-sdk/es/academy/level1_policy/index.md)
...
En el Nivel 1, arreglaste el sesgo localmente. Pero tu manager lo niega porque no puede ver la prueba. Los emails con capturas de pantalla **no son cumplimiento**.
...
1. **Chequeo de Logs**: Verifica que `.venturalitica/results.json` existe.

1. **Navega a "Estado de Política"**: Confirma que tu "Tratamiento de Riesgo" (el umbral ajustado) está registrado.
...
1. **Nivel 2 (Artículo 15)**: Verifica el **Comportamiento del Modelo** contra `model_policy.yaml`. El objetivo es probar que la IA toma decisiones justas en una ejecución de "Caja de Cristal".
...
Si `compliance_score == 0`, la build falla. GitLab CI / GitHub Actions ahora pueden bloquear un despliegue basado en ética, igual que bloquean por errores de sintaxis.
...
1. **Telemetría es Evidencia**: RAM, CO2 y Traza no son solo métricas—cumplen con la supervisión del **Artículo 15**.
1. **Traza Unificada**: `vl.monitor()` captura todo, desde uso de hardware hasta análisis de código AST, en un solo archivo `.json`.
1. **Fricción Cero**: El Data Scientist sigue usando MLflow/WandB, mientras el SDK cosecha la evidencia.

👉 **[Siguiente: Nivel 3 (El Auditor)](https://venturalitica.github.io/venturalitica-sdk/es/academy/level3_auditor/index.md)**
