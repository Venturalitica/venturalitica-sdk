...
1. **Control de Política**: "La Tasa de Impacto Dispar debe ser > 0.8".
1. **Aserción de Código**: `assert calculated_metric > 0.8`.
...
```
[Venturalítica v0.4.1] 🎓 Escenario: Equidad en Calificación Crediticia
...
1. **Carga Política**: Lee `risks.oscal.yaml` que define las reglas de equidad.
1. **Ejecuta**: Corre la auditoría (`vl.enforce`).
1. **Registra**: Captura la evidencia (`trace.json`) para el panel de control.
...
1. Selecciona **"Español"**.
1. Haz clic en **"Generar Anexo IV"**.
...
- **[Referencia de API](https://venturalitica.github.io/venturalitica-sdk/es/api/index.md)** - Documentación completa
