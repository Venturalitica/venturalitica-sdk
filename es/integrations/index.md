...
Esta guía se centra en la **Persona MLOps**: quien automatiza el pipeline. Venturalítica se integra estrictamente con tus herramientas existentes para asegurar que la **Evidencia (Artículo 12)** se recopile automáticamente durante tus ejecuciones de CI/CD.
...
1. **Ejecutar**: Solo usa `vl.wrap(model)` dentro de tu script.
1. **Verificar**: Abre `venturalitica ui` -> **Integraciones**.
...
1. **Ejecutar**: Asegúrate de que `mlflow.start_run()` esté activo cuando llames a `fit()`.
1. **Verificar**: La UI generará enlaces profundos a tu Experimento y ID de Ejecución específicos.
