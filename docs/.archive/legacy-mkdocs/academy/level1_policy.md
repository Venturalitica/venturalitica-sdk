...
| :--- | :--- | :--- |
| `metric_key` | Which metric to compute (from [Metrics Reference](../metrics.md)) | `disparate_impact`, `class_imbalance`, `accuracy_score` |
...
| `input:dimension` | Protected attribute to slice by | `gender`, `age` (resolved via [Column Binding](../column-binding.md)) |
...
Two controls pass, one fails. The age disparity ratio (0.361) is below the 0.5 threshold.
...
See [Column Binding](../column-binding.md) for the full resolution algorithm.
...
- A red flag for `credit-age-disparate` with the measured value (0.361) vs. threshold (0.5)
