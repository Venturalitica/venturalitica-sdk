...
Any metric registered in `METRIC_REGISTRY` can be used as a `metric_key`. See [Metrics Reference](metrics.md) for the full list. Common ones:
...
The `input:dimension` prop tells the SDK which protected attribute to analyze. The value is an abstract name that gets resolved via [Column Binding](column-binding.md):
...
!!! warning "SSP Format Inconsistency"
    The CLI `pull` command currently downloads policies in `system-security-plan` format. These load correctly but differ from the `assessment-plan` format generated by the Dashboard. A future release will unify this.
