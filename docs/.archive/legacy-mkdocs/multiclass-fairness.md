...
**Note**: This metric returns a dictionary, not a float. It is primarily for diagnostic reporting and may not work directly with threshold-based OSCAL controls. Use `calc_multiclass_fairness_report()` in Python for combined analysis.
...
**Note**: This metric returns a tuple `(value, metadata)` where metadata contains `total_samples`, `min_class_support`, and `min_prediction_support`.
...
- **[Metrics Reference](metrics.md)** -- All 35+ metrics including binary fairness, privacy, and performance
- **[Policy Authoring](policy-authoring.md)** -- How to use metric keys in OSCAL controls
- **[Column Binding](column-binding.md)** -- How `dimension`, `target`, `prediction` map to columns
