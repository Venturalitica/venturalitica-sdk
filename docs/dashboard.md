...
The **Venturalitica Dashboard** is your local compliance workspace. It guides you through 4 phases of the EU AI Act compliance lifecycle without leaving your terminal.
...
The dashboard follows a **4-phase compliance journey** mapped to EU AI Act requirements:
...
The home screen presents 4 missions as a progress dashboard. Each mission shows its completion status:

| Mission | Status Check | Description |
| :--- | :--- | :--- |
...
| 3. Verify Evidence | `results.json` or `trace_*.json` exists | Telemetry, traces, and metric validation |
| 4. Document | `venturalitica_technical_doc.json` exists | Generated Annex IV technical file |

Click any mission card to navigate directly to that phase.
...
Define the "ground truth" of your AI system using the **Model Card Editor**. This creates `system_description.yaml` with:
...
The **Policy Editor** lets you create and edit OSCAL policy files visually. It generates `assessment-plan` format OSCAL YAML with:
...
!!! tip
    The Policy Editor generates `assessment-plan` format, which is the canonical OSCAL format used throughout the SDK. See [Policy Authoring Guide](policy-authoring.md) for the manual YAML format.
...
- Per-control compliance results with pass/fail status
...
!!! warning "ALIA Requirements"
    ALIA is a 40B parameter model requiring ~41GB RAM/VRAM. It is experimental.
